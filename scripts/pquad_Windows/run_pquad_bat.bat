...
echo from pquad import get_defaults>>temp.py
echo from pquad import IR_Results>>temp.py
echo from pquad.plotting_tools import set_figure_settings>>temp.py
echo.>>temp.py
...
echo Respond no if you want to use the default pure data to train the model.: ').strip('\' \^"')>>temp.py
echo if use_your_own.lower() in ['yes', 'y']:>>temp.py
echo     pure_data_path = input('Please enter the directory to the pure-species data file: ').strip('\' \^"')>>temp.py
echo     frequency_start = input('Please enter the lowest frequency to consider: ').strip('\' \^"')>>temp.py
echo     frequency_end = input('Please enter the highest frequency to consider: ').strip('\' \^"')>>temp.py
echo     pca_to_keep = input('Please enter the number of principal componets in the spectra to keep. \>>temp.py
echo     A good starting number is the number of pure-components: ').strip('\' \^"')>>temp.py
echo     frequency_range = np.linspace(float(frequency_start),float(frequency_end),num=501,endpoint=True)>>temp.py
echo mixture_data_path = input('Please enter the directory to the mixture data: ').strip('\' \^"')>>temp.py
echo output_folder = input('Please enter the directory to the save the data: ').strip('\' \^"')>>temp.py
...
echo a parity plot is made. If False, the data is considred reaction data.: ').strip('\' \^"')>>temp.py
echo if True_or_False.lower() in ['yes', 'y', 'true', 't']:>>temp.py
echo     True_or_False = True>>temp.py
echo elif True_or_False.lower() in ['no', 'n', 'false', 'f']:>>temp.py
echo     True_or_False = False>>temp.py
echo contains_concentrations = True_or_False>>temp.py

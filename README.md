...
`jl_exp_deconv/jl_exp_deconv.py`. The code provided in this file is intentionally rather
...
`jl_exp_deconv`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import jl_exp_deconv as sb` in an
interactive Python session, the classes and functions defined inside of the
`jl_exp_deconv.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the jl_exp_deconv module, the main classes defined at the bottom of
...
Either way, you can create a `jl_exp_deconv/data` folder in which you can
...
the top-level directory of the repository (e.g. `/Users/arokem/code/jl_exp_deconv`),
and type:
...
will see a message such as:
    ...
    jl_exp_deconv/tests/test_jl_exp_deconv.py .F...
    ...
    jl_exp_deconv/tests/test_jl_exp_deconv.py:49: AssertionError
...
page](https://pypi.python.org/pypi/jl_exp_deconv) for the software. This
...
`jl_exp_deconv` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `jl_exp_deconv`, we use the
...
[here](https://github.com/uwescience/jl_exp_deconv/blob/master/scripts/Figure1.ipynb).
...
### Using `jl_exp_deconv` as a template
...
button on the front page of the "jl_exp_deconv" repository.
...
`jl_exp_deconv/jl_exp_deconv.py` to be called `smallish/smallish.py`

	git mv jl_exp_deconv smallish
	git mv smallish/jl_exp_deconv.py smallish/smallish.py
	git mv smallish/tests/test_jl_exp_deconv.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `jl_exp_deconv` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `jl_exp_deconv` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `jl_exp_deconv` in them. Type
the following to see where all these files are:

	git grep jl_exp_deconv

You can replace `jl_exp_deconv` for `smallish` quickly with:

	git grep -l 'jl_exp_deconv' | xargs sed -i 's/jl_exp_deconv/smallish/g'
...
Edit all the mentions of `jl_exp_deconv` in the `jl_exp_deconv/__init__.py` file, and in
the `jl_exp_deconv/version.py` file as well.
